...
async function listDomains() {
  try {
    ...
    domains.data.forEach((domain) => {
      console.log(`   - ${domain.domain_name} (ID: ${domain.id}, ステータス: ${domain.status})`);
    });
    return domains.data;
  } catch (error: any) {
    console.error('❌ ドメイン一覧の取得に失敗しました:', error.message);
    return [];
  }
}
...
async function main() {
  ...
  try {
    ...
    const isTopLevelDomain = !domain.includes('.') || domain.split('.').length === 2;
