...
const variables = [
  ...
  {
    name: '{grantedPoints}',
    description: 'ガチャ景品として付与された無償ポイント数（ポイント付与がない場合は0）',
    example: '例: 「100」「500」',
  },
  {
    name: '{grantedPointsMessage}',
    description: 'ポイント付与メッセージ（ポイント付与がある場合のみ表示される文字列）',
    example: '例: 「💰 無償ポイント 100ポイントが付与されました！」',
  },
];

export default function VariableInfoModal({ isOpen, onClose }: VariableInfoModalProps) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-semibold text-gray-900">使用可能な変数</h3>
          <button
            onClick={onClose}
            ...
            <div key={index} className="rounded-lg border border-gray-200 p-4 bg-gray-50">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-xs text-gray-600">{variable.example}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="mt-6 pt-4 border-t border-gray-200">
          <p className="text-xs text-gray-500">
            ...
            <br />
            ポイント付与がない場合は、{'{grantedPoints}'}は0、{'{grantedPointsMessage}'}は空文字になります。
          </p>
        </div>
