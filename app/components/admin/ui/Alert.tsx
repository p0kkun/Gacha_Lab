    ...
    >
      <div className="flex">
        ...
        <div className="ml-3 flex-1">
          ...
          <div className={`text-sm ${title ? 'mt-2' : ''} ${styles.text}`}>{children}</div>
        </div>
        {onClose && (
          <div className="ml-auto pl-3">
            <button
              onClick={onClose}
              className={`inline-flex rounded-md ${styles.text} hover:bg-opacity-20 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-${variant === 'error' ? 'red' : variant === 'success' ? 'green' : variant === 'warning' ? 'yellow' : 'blue'}-500`}
            >
              <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
